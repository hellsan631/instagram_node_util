...
program
  ...
  .option('-t, --text_output [text_output]', "generate output with textual data", false)
  .option('-t, --text_input [text_input]', "list of csv files, comma separated, output CSV file with text training data", false)
  .option('-c, --customimport [csv]', "csv file for explicit imports, if set other imports are ignored.")
  .option('-a, --accountnames [bool]', "If true converts realname->accountname.", false)
  .option('-w, --write_frequency [count]', "Sample count to dump csv file.", 200)
  .parse(process.argv)
  ...
  while (retryConnect) {
    try {
      ...
      if (program.accountnames) {
        if (!await getAccountFromName(name, account)) {
          // skip, no results.
          return false;
        }
      } else {
...
async function updateMissingCounters(accountDict) {
  ...
  for (let name in accountDict) {
    ...
    if (writeCounter <= 0) {
      ...
      writeCounter = program.write_frequency;
      if (program.text_output || program.accountnames) {
...
async function getAccountFromName(realName, account) {
  ...
  if (!response.users || response.users.length == 0) {
    console.log('no results for:', realName);
    return false;
  }
...
/* alternate way of reading this:
https://stackoverflow.com/questions/49812575/instagram-a-1-not-working-anymore
*/
// TODO(tvykruta): Add support to extract tags from image. Roughly:
// shortcode_media.edge_media_to_tagged_user.edges[...].node.user.username
async function processAccount(accountName, account) {
  const url = "https://apinsta.herokuapp.com/u/" + accountName;
  let response = await getFbData(accountName, url);
  // console.log('fetched:\n', JSON.stringify(response, null, 2));
...
async function main() {
  ...
  if (program.text_input) {
    try {
      const files = program.text_input.split(',');
      for (const file of files) {
        await readCsvAndMergeCounts(
          file,
          dictCounts,
